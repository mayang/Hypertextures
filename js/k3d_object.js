...
(function()
{
   ...
   extend(K3D.K3DObject, K3D.BaseObject,
   {
      ...
      // MY STUFF FOR HYPERTEXTURES
      ht_dmf: null, // type of hypertexture, default null is no hypertexture to apply
      freq: 0, // for noise!
      ...
      init: function(points, edges, faces)
      {
         ...
         // calculate D's for each point on sphere
         // soft area only surface cuz I am a lazy fuck.
         switch(this.ht_dmf) {
             case "noise": {
                BasicNoise(points, this.freq);
                console.log(points[100].D); // to see if this works!
                break;
             }
             default: break;
         }
         ...
         if (this.recalculateNormals)
         {
            ...
            for (var i=0, j=faces.length, vertices, x1, y1, z1, x2, y2, z2; i<j; i++)
            {
               ...
               z2 = pts[vertices[2]].z - pts[vertices[0]].z;
                    // save the normal vector as part of the face data structure
                faces[i].normal = calcNormalVector(x1, y1, z1, x2, y2, z2);
                // also check to do other calc norm
